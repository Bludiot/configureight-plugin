...
function uploadImage( $pluginPath, $albumDir, $config ) {
	...
	$imageSettings = [
		'thumb' => [
			'cacheName' => 'thumb',
			'width'     => $config->getField( 'logo_thumb_width' ),
			'height'    => $config->getField( 'logo_thumb_height' ),
			'format'    => 'crop',
			'quality'   => $config->getField( 'img_upload_quality' )
		],
		'large' => [
			'cacheName' => 'large',
			'width'     => $config->getField( 'logo_large_width' ),
			'height'    => $config->getField( 'logo_large_height' ),
			'format'    => null,
			'quality'   => $config->getField( 'img_upload_quality' )
		]
	];
